	...
	class TripAjax {
		...
		public function render_trip_dates() {
			...
			foreach ( $trip_dates as $trip_date ) :
				...
				if ( isset( $trip_date['custom_price'] ) ) {
					$custom_price = is_scalar( $trip_date['custom_price'] ) ? trim( (string) $trip_date['custom_price'] ) : '';
					if ( '' !== $custom_price ) {
						$normalised_price = preg_replace( '/[^0-9\\,\\.]/', '', $custom_price );
						$normalised_price = str_replace( ',', '.', $normalised_price );
						if ( is_numeric( $normalised_price ) ) {
							$custom_price_value = (float) $normalised_price;
						}
					}
				}
				$display_price = null !== $custom_price_value ? $custom_price_value : $price;
				...
				?>
				<div class="tripzzy-dates-content <?php echo esc_attr( $content_class ); ?>" data-initial-price="<?php echo esc_attr( Amount::display( $display_price ) ); ?>" <?php echo esc_attr( $times_data ); ?> data-extras="<?php echo esc_attr( $trip_extras_data ); ?>"
				data-trip-booking='<?php echo esc_attr( wp_json_encode( $booking_data ) ); ?>'> <!-- Repeator -->
					...
					<ul>
						...
						<li class="tz-departure-list-price-wrapper">
							...
							<strong class="tz-departure-list-from-price"><?php echo esc_html( number_format_i18n( $display_price ) ); ?> €</strong><span> / <?php echo esc_html( $price_per ); ?></span>
						</li>
						<li class="tz-departure-list-book-now-wrapper">
							<div class="tz-departure-list-book-now">
								<button type="button" class="tripzzy__booking-button tz-btn tz-btn-outline tz-btn-full" data-tripzzy-drawer-trigger aria-controls="tripzzy-enquiry-form-wrapper" aria-expanded="false">
									<?php echo esc_html__( 'Demande de devis', 'tripzzy' ); ?>
								</button>
							</div>
						</li>
					</ul>
