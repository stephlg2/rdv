...
if ( ! class_exists( 'Tripzzy\Core\Helpers\Loading' ) ) {
	/**
	 * Class For Page
	 *
	 * @since 1.0.0
	...
	class Loading {
		/**
		 * Render the loading html.
		 *
		 * @param array $args Loader arguments.
		 * @param bool  $has_return whether return or echo.
		 *
		 * @return markup
		 */
		public static function render( $args = array(), $has_return = false ) {
			$class = $args['class'] ?? array();
			$id    = $args['id'] ?? '';
			ob_start();
			?>
			<div class="tripzzy-loader <?php echo esc_attr( implode( ' ', $class ) ); ?>" id="<?php echo esc_attr( $id ); ?>" >
