    ...
    <?php if (!empty($devis_list)) : ?>
        <div class="devis-list-client">
            <?php foreach ($devis_list as $devis) : 
                ...
                // Si pas d'URL trouvée à partir des IDs, essayer de trouver une page de destination
                if (empty($voyage_url) && !empty($devis->destination)) {
                    // Chercher une page de destination correspondante
                    $destination_slug = sanitize_title($devis->destination);
                    // Essayer différentes structures d'URL possibles
                    $possible_urls = array(
                        home_url('/circuit-voyage-asie/' . $destination_slug . '/'),
                        home_url('/voyage-en-asie/' . $destination_slug . '/'),
                        home_url('/destinations/' . $destination_slug . '/'),
                    );
                    foreach ($possible_urls as $url) {
                        // Vérifier si la page existe (optionnel, on peut juste créer le lien)
                        $voyage_url = $url;
                        break; // Utiliser le premier format
                    }
                }
                
                // Si toujours pas d'URL, créer un lien vers la page des destinations
                if (empty($voyage_url)) {
                    $voyage_url = home_url('/circuit-voyage-asie/toutes-destinations-asie/');
                }
            ?>
