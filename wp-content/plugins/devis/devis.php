...
function devis_creation($atts){
    ...
    if($values['send'] && $values['email'] != "" && $values['tel']){
        ...
        include_once( 'views/devis-envoye.php' );
    }else{
        $error = "";
        if($values['send']){
...
function devis_paiement(){
    ...
    if($values['q']){
        ...
        include_once( 'views/paiement/index.php');
    }else{
...
add_shortcode('paiement-devis', 'devis_paiement');
