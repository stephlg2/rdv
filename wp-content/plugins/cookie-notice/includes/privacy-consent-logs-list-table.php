...
class Cookie_Notice_Privacy_Consent_Logs_List_Table extends WP_List_Table {
	...
	public function views() {
		...
		$message .= ' ' . sprintf( __( 'Log in to the <a href="%s" target="_blank">Cookie Compliance</a> Admin Portal to view details or export proof of consent.', 'cookie-notice' ), $login_url );
		
		// disable if basic plan and data older than 7 days
		if ( $cn->get_subscription() === 'basic' )
			$message .= '<br/><span class="cn-asterix">*</span> ' . __( 'Note: sites on the Cookie Compliance Free Plan can collect up to 100 records.', 'cookie-notice' );
