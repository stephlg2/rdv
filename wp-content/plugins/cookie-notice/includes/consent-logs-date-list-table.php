...
class Cookie_Notice_Consent_Logs_Date_List_Table extends WP_List_Table {
	...
	public function views() {
		...
		$message .= ' ' . sprintf( __( 'View individual records by expanding a single row of data or log in to the <a href="%s" target="_blank">Cookie Compliance</a> Admin Portal to export proof of consent.', 'cookie-notice' ), $login_url );

		// disable if basic plan and data older than 7 days
		if ( $cn->get_subscription() === 'basic' )
			$message .= '<br/><span class="cn-asterix">*</span> ' . __( 'Note: sites on the Cookie Compliance Free Plan can view consent records from the last 7 days and store data only for 30 days.', 'cookie-notice' );
