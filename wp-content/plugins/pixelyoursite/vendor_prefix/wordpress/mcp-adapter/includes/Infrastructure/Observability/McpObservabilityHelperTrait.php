...
namespace PYS_PRO_GLOBAL\WP\MCP\Infrastructure\Observability;
...
trait McpObservabilityHelperTrait
{
     ...
     */
    private static array $error_categories = array(\ArgumentCountError::class => 'arguments', \Error::class => 'system', \InvalidArgumentException::class => 'validation', \LogicException::class => 'logic', \RuntimeException::class => 'execution', \TypeError::class => 'type');
    /**
     ...
     * @var string[]
     */
    private static array $sensitive_patterns = array('password', 'passwd', 'pwd', 'secret', 'token', 'bearer', 'credential', 'private', 'apikey', 'api_key', 'authtoken', 'auth_token', 'accesstoken', 'access_token', 'refreshtoken', 'refresh_token', 'clientsecret', 'client_secret', 'privatekey', 'private_key', 'secretkey', 'secret_key', 'authorization', 'authenticate', 'encryption');
    /**
    ...
    public static function format_metric_name(string $metric): string
    {
        ...
        $metric = (string) preg_replace('/\.+/', '.', $metric);
    ...
    {
        return array('site_id' => function_exists('PYS_PRO_GLOBAL\get_current_blog_id') ? get_current_blog_id() : 0, 'user_id' => function_exists('get_current_user_id') ? get_current_user_id() : 0, 'timestamp' => time());
    }
    /**
